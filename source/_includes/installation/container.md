...
* `confpath` points at the folder where you want to store your configuration and run it (the example uses `$HOME/HomeAssistant` as a default).
* `tz` is a [tz database name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones), like `TZ=America/Los_Angeles` (the example uses the contents of your system-wide `/etc/timezone` file as a default).
